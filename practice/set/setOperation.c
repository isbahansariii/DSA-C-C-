...
void duplication(int num)
{
    // inserted data in list 3 but does not insert duplicat
    struct node *current = set3;
    while (current != NULL)
    {
        // if number is repeating, return without storing
...
void Union(struct node *S1, struct node *S2)
{
    if (S1 == NULL && S2 == NULL)
    {
        printf("\nSet is NULL.");
